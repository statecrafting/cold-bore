...
cold-bore is a fault-injectable model data platform: edge simulator -> RabbitMQ -> Rust ingest -> TimescaleDB -> Python API/WebSocket -> dashboard, governed by spec-spine (installed binary).

| Surface | Path | Tech |
|---------|------|------|
| Spec corpus | `specs/NNN-slug/spec.md` | Markdown + YAML frontmatter |
| Design doc | `docs/design/architecture.md` | Topology, delivery contract, drill matrix |
| Rust crates | `crates/{coldbore-proto,coldbore-edge,coldbore-ingest}/` | Shared types, pad simulator/publisher, consumer/sink |
| Python API | `services/api/` | FastAPI + aio-pika + asyncpg (uv-managed) |
| Dashboard | `dashboard/` | No-build-step JS |
...
Key files: `CLAUDE.md` (conventions, delivery invariants), `AGENTS.md` (session protocol), `.claude/rules/` (behavioral rules).
