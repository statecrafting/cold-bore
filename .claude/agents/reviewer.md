...
|---------|------|--------------|
| Spec corpus | `specs/NNN-slug/spec.md` | Frontmatter schema, relationship edges, status flips |
| Rust crates | `crates/{coldbore-proto,coldbore-edge,coldbore-ingest}/` | Delivery invariants (ack/offset after commit, idempotent sink, bounded buffers, counted drops), error handling, no panics on broker/DB input |
| Python API | `services/api/` | Async correctness, connection lifecycle, WS backpressure, SQL safety |
| Cross-language contracts | frame/control/metrics/event JSON | Rust and Python and the architecture doc must agree |
| Infra | `infra/` | Durability flags, resource caps, migration idempotency |
